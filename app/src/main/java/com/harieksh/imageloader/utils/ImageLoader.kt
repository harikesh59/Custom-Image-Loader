package com.harieksh.imageloader.utils
...
class ImageLoader(context: Context) {
    ...
    init {
        ...
        executorService = Executors.newFixedThreadPool(5, Utils.ImageFactory())
        handler = Handler()
    ...
    fun placeHolder(imageView: ImageView, imgResource: Drawable) {

        require(imageView != null) {
            "ImageLoader:load - ImageView should not be null."
        }
        imageView.setImageDrawable(imgResource)
    }
    ...
    private fun loadImageIntoImageView(imageView: ImageView, bitmap: Bitmap?, imageUrl: String) {
        ...
        val scaledBitmap = Utils.scaleBitmap(bitmap, imageView.width, imageView.height)
    ...
    inner class PhotosLoader(private var imageRequest: ImageRequest) : Runnable {
        ...
        override fun run() {
            ...
            val bitmap = Utils.downloadBitmap(imageRequest.imgUrl)
